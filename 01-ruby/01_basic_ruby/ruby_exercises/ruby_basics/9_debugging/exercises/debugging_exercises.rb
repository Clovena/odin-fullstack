  ...
  nested_array.each do |array|
    array.each do |current_value|
      if smallest_value > current_value
        smallest_value = current_value # fixed typo: current_valu becomes current_value
      end
    end
  end
  smallest_value -= 1
...
def increment_greatest_value(nested_array)
  ...
  nested_array.each do |array|
    array.each do |current_value|
      if greatest_value < current_value # fixed comparison with nil; replaced nil with current_value
...
def isogram?(string)
  ...
  string_array = string.downcase.split '' # split requires an argument in this instance
...
def yell_greeting(string)
  name = string
  # debugger
  name = name.upcase # replaced #downcase with #upcase
